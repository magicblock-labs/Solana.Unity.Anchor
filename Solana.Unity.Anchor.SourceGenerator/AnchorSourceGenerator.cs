...
namespace Solana.Unity.Anchor.SourgeGenerator
{
    [Generator]
    public class AnchorSourceGenerator : ISourceGenerator
    {
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            if (!success)
            {
                context.ReportDiagnostic(Diagnostic.Create(new DiagnosticDescriptor("ANCHOR-GENERATOR-01", "Tool Error", "dotnet anchorgen not found. Please install the tool using 'dotnet install Solana.Unity.Anchor.SourceGenerator'", "Tool.Dependency", DiagnosticSeverity.Error, true), null));
                return;
            }
