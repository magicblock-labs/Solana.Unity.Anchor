...
namespace AuctionHouse
{
    ...
    public partial class AuctionHouseClient : TransactionalBaseClient<AuctionHouseErrorKind>
    {
        ...
        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<AuctionHouse>>> GetAuctionHousesAsync(string programAddress, Commitment commitment = Commitment.Finalized)
        {
            var list = new List<Solana.Unity.Rpc.Models.MemCmp>{new Solana.Unity.Rpc.Models.MemCmp{Bytes = AuctionHouse.ACCOUNT_DISCRIMINATOR_B58, Offset = 0}};
            var res = await RpcClient.GetProgramAccountsAsync(programAddress, commitment, memCmpList: list);
            if (!res.WasSuccessful || !(res.Result?.Count > 0))
                return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<AuctionHouse>>(res);
            ...
            return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<AuctionHouse>>(res, resultingAccounts);
        }
        ...
        public async Task<Solana.Unity.Programs.Models.AccountResultWrapper<AuctionHouse>> GetAuctionHouseAsync(string accountAddress, Commitment commitment = Commitment.Finalized)
        {
            var res = await RpcClient.GetAccountInfoAsync(accountAddress, commitment);
            if (!res.WasSuccessful)
                return new Solana.Unity.Programs.Models.AccountResultWrapper<AuctionHouse>(res);
            var resultingAccount = AuctionHouse.Deserialize(Convert.FromBase64String(res.Result.Value.Data[0]));
            return new Solana.Unity.Programs.Models.AccountResultWrapper<AuctionHouse>(res, resultingAccount);
        }
        ...
        public async Task<SubscriptionState> SubscribeAuctionHouseAsync(string accountAddress, Action<SubscriptionState, Solana.Unity.Rpc.Messages.ResponseValue<Solana.Unity.Rpc.Models.AccountInfo>, AuctionHouse> callback, Commitment commitment = Commitment.Finalized)
        {
            SubscriptionState res = await StreamingRpcClient.SubscribeAccountInfoAsync(accountAddress, (s, e) =>
            {
